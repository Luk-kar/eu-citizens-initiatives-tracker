...
- **Dataset 1 (Base)**: Main ECI response data scraped from official Commission response pages (`eci_responses_*.csv`)
- **Dataset 2 (Followup)**: Updated implementation data from dedicated follow-up websites (`eci_responses_followup_website_*.csv`)
...
Dataset 1 captures the Commission's initial response with specific deadlines, policy commitments, and procedural milestones—representing what was promised. Dataset 2 reflects the evolving reality of implementation captured from follow-up websites over time—showing what was actually delivered.
...
- **Merge Strategy**: Primary key - keep from Dataset 1
...
#### **2. initiative_title**
- **Merge Strategy**: Keep from Dataset 1
- **Conflict Resolution**: N/A (identical in both)
...
### **Unique to Dataset 1 (14 columns) - Keep All**

#### **3. response_url**
- **Merge Strategy**: Keep from Dataset 1 (no equivalent in Dataset 2)
...
- **Merge Strategy**: Keep Dataset 1 (identical in both)
...
- **Merge Strategy**: Merge Dataset 1 with Dataset 2 (when available)
- **Conflict Resolution**: When conflict, add title for Dataset 1: **Original Commission Commitments:** and title for Dataset 2: **Current Commission Commitments:**
- **Null Handling**: If Dataset 2 is null, keep Dataset 1
- **Context & Decision**: Dataset 1 captures the original Commission response with specific deadlines and policy framing (e.g., "by end of 2023"), while Dataset 2 reflects evolved implementation details from follow-up websites (e.g., actual EFSA opinion publication dates). Both provide accountability value—original promises versus actual delivery—so preserve both with clear labeling to track commitment versus implementation.

#### **19. official_communication_document_urls**
- **Merge Strategy**: Merge (union) - combine unique URLs from both sources
- **Conflict Resolution**: Parse JSON/dict structure, combine all unique URLs with descriptive keys; when URLs recur in different keys, preserve the key from Dataset 1
- **Null Handling**: If either is null, use the other
- **Context & Decision**: Dataset 1 provides official Commission documents (Communications, main press releases, Q&A PDFs) while Dataset 2 adds contextual links from follow-up websites (earlier press releases from submission phase, navigation paths). Combining creates comprehensive documentation trail spanning submission → response → follow-up phases.

#### **20. final_outcome_status**
- **Merge Strategy**: Prioritize Dataset 2 (when available, with validation)
- **Conflict Resolution**: Dataset 2 appears more current (e.g., "Being Studied" → "Law Approved" for 2022/000002)
- **Null Handling**: If Dataset 2 is null, keep Dataset 1
...
- **Context & Decision**: Outcome status evolves as Commission implements (or doesn't implement) commitments over years; follow-up websites are updated more frequently than main ECI portal. Dataset 2 reflects current reality, but validation prevents illogical transitions.

#### **21. law_implementation_date**
- **Merge Strategy**: Keep Dataset 1 if Dataset 2 is null; update with Dataset 2 when exists
- **Conflict Resolution**: Update with Dataset 2 when exists
- **Null Handling**: Preserve nulls
- **Context & Decision**: Implementation dates become known only after legislation is adopted and enters into force; follow-up websites capture these actual dates as they occur. Prioritize Dataset 2 as it reflects observed reality rather than projections.

#### **22. commission_promised_new_law**
- **Merge Strategy**: When Dataset 1 is False and Dataset 2 is True set True; if both False then False; if Dataset 1 is True and Dataset 2 is False keep True
- **Conflict Resolution**: N/A
...
- **Conflict Resolution**: Parse JSON, merge all unique date-action pairs; when conflict, add title for Dataset 1: **Original Commission Deadlines:** and title for Dataset 2: **Current Commission Deadlines:**
- **Null Handling**: If either is null, use the other; if both have data, merge
- **Context & Decision**: Original deadlines represent Commission accountability commitments at response time, while follow-up websites add intermediate milestones (e.g., consultation closures) and updated timeline language. Example: 2022/000002 original "March 2026 decision" becomes more specific in Dataset 2 with August 2025 consultation deadline added plus refined March 2026 language.

#### **24. commission_rejected_initiative**
- **Merge Strategy**: When Dataset 1 is True keep it; when Dataset 1 is False and Dataset 2 is True keep Dataset 2
- **Conflict Resolution**: Dataset 1 overwrites Dataset 2
- **Null Handling**: N/A (boolean)
- **Context & Decision**: Commission rejections are permanent political decisions that cannot be reversed (as demonstrated by cases like Minority SafePack and One of Us remaining rejected 7-13 years later). This field follows one-way logic where rejection status can only be added, never removed, with Dataset 1 priority as the authoritative source.

#### **25. commission_rejection_reason**
- **Merge Strategy**: Combine both Dataset 1 and Dataset 2
- **Conflict Resolution**: When conflict, add title for Dataset 1: **Original Commission Rejection Reason:** and title for Dataset 2: **Current Commission Rejection Reason:**
- **Null Handling**: Preserve nulls
...
- **Null Handling**: If Dataset 1 is null, use Dataset 2; otherwise merge arrays
- **Context & Decision**: Dataset 1 often has null or only initial legislative promises, while Dataset 2 adds concrete later actions like specific regulation adoptions with CELEX numbers (e.g., 2022/000002 Dataset 2 adds July 2025 Invasive Alien Species Regulation). Combining creates complete legislative outcome timeline from promise to implementation.

#### **27. policies_actions**
- **Merge Strategy**: Merge (append with deduplication)
- **Conflict Resolution**: Parse JSON arrays, deduplicate by (type + date + description)
- **Null Handling**: If either is null, use the other
- **Context & Decision**: Both datasets contain policy actions but Dataset 2 typically has more granular detail (specific consultation dates, named research projects) versus Dataset 1's broader categories. Merging creates the most comprehensive non-legislative action timeline.
...
- **Conflict Resolution**: N/A
- **Null Handling**: Preserve nulls
- **Context & Decision**: References to court cases challenging the Commission's ECI response (like Minority SafePack's General Court and CJEU challenges); combining sources ensures comprehensive legal challenge documentation, though currently null for the two ECIs in Dataset 2.
...
- **Conflict Resolution**: Parse JSON arrays, deduplicate by same key and value; when conflict, add title for Dataset 1: **Original Commission Event:** and title for Dataset 2: **Current Commission Event:**
- **Null Handling**: If either is null, use the other
- **Context & Decision**: Dataset 1 captures historical events from initial scraping while Dataset 2 adds more recent events and updated descriptions from follow-up websites. Both provide valuable temporal data for complete accountability timeline, with conflicts preserved to show evolution of event descriptions over time (e.g., generic vs. specific details).
...
- **Total columns**: 36
- **Identity columns**: 2 (keep Dataset 1)
- **Unique to Dataset 1**: 14 (keep all)
- **Overlapping columns**: 20 (merge with various strategies)
  - Keep Dataset 1 only: 1
  - Merge/union: 11
  - Prioritize Dataset 2 with validation: 2
  - Logical OR (booleans): 3
  - Complex merge with labeling: 3

**Merge philosophy**: Dataset 1 provides authoritative historical record of Commission's initial response; Dataset 2 adds implementation reality and updated details from follow-up websites. Combining creates comprehensive accountability dataset tracking Commission commitments versus actual delivery over time.
