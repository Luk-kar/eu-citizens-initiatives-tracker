...
class TestCreatedFiles:
    ...
    def setup_class(cls):
        ...
        from ECI_initiatives.data_pipeline.extractor.initiatives.processor import (
            ECIDataProcessor,
        )
    ...
    def test_log_file_naming_pattern(self):
        ...
        assert log_filename.startswith(
            "extractor_initiatives_"
        ), f"Log filename should start with 'extractor_initiatives_', got: {log_filename}"

        assert log_filename.endswith(
            ".log"
        ), f"Log filename should end with '.log', got: {log_filename}"

        # Extract timestamp part
        timestamp_part = log_filename.replace("extractor_initiatives_", "").replace(
            ".log", ""
        )

        # Pattern: YYYY-MM-DD_HH-MM-SS
        assert (
            len(timestamp_part) == 19
    ...
    def test_log_file_contains_entries(self, program_root_dir):
        ...
        test_data_dir = (
            program_root_dir / "tests" / "data" / "example_htmls" / "initiatives"
        )
        ...
        processor = self.ECIDataProcessor(
            data_root=str(self.temp_session_path), logger=None
        )
        ...
        with patch.object(
            processor, "find_latest_scrape_session", side_effect=mock_find_session
        ):
            processor.run()
        ...
        assert (
            "Starting ECI data processing" in log_content or "Processing" in log_content
    ...
    def test_csv_created_with_zero_initiatives(self):
        ...
        if len(log_files) > 0:
            ...
            assert (
                "No initiatives to save" in log_content
