...
class TestRequiredFieldValidation:
    ...
    def test_registration_number_never_none(self, program_root_dir):
        ...
        with patch.object(
            ECIDataProcessor, "find_latest_scrape_session", return_value=session_dir
        ):
        ...
        with open(csv_files[0], "r", encoding="utf-8") as f:
            ...
            for row in reader:
                assert row[
                    "registration_number"
                ], "registration_number should not be None or empty"
                assert (
                    row["registration_number"] != "None"
                ), "registration_number should not be literal 'None'"
