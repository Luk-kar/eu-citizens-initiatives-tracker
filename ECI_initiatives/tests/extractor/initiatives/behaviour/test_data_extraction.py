...
# Local
from ECI_initiatives.data_pipeline.extractor.initiatives.parser import ECIHTMLParser
from ECI_initiatives.data_pipeline.extractor.initiatives.initiatives_logger import (
    InitiativesExtractorLogger,
)
...
class TestRegistrationNumberExtraction:
    ...
    def test_valid_filename_format(self):
        ...
        assert (
            reg_number == expected_reg_number
    ...
    def test_registration_number_formatting(self):
        ...
        reg_number = self.parser._extract_registration_number(filename)
        assert (
            expected_separator in reg_number
        ), f"Registration number should contain '{expected_separator}'"
        assert (
            len(reg_number) == expected_number
        ), f"Registration number should be {expected_number} characters (YYYY/NNNNNN)"
...
class TestObjectiveExtraction:
    ...
    def test_objective_character_limit(self):
        ...
        assert (
            len(objective) == expected_length
        ), f"Objective should be {expected_length} chars, got {len(objective)}"
    ...
    def test_objective_multi_paragraph(self):
        ...
        assert (
            expected_first_paragraph in objective
        ), f"Should contain '{expected_first_paragraph}'"
        assert (
            expected_second_paragraph in objective
        ), f"Should contain '{expected_second_paragraph}'"
...
class TestURLConstruction:
    ...
    def test_url_construction_format(self):
        ...
        expected_url = (
            "https://citizens-initiative.europa.eu/initiatives/details/2024/000005_en"
        )
    ...
    def test_url_contains_registration_parts(self):
        ...
        assert (
            expected_number in url
        ), f"URL should contain initiative number '{expected_number}'"
...
class TestSignaturesExtraction:
    ...
    def test_signatures_collected_from_table(self):
        ...
        ), f"Expected '{expected_signatures}', got '{signatures}'"
