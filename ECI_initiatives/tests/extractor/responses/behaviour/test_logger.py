...
class TestLoggerInitialization:
    ...
    def test_logger_name(self):
        ...
        assert (
            logger_instance._logger.name == expected_name
        ), f"Logger name should be '{expected_name}'"
    ...
    def test_multiple_instances_share_underlying_logger(self):
        ...
        assert (
            logger_instance_1 is not logger_instance_2
...
class TestLoggerConfiguration:
    ...
    def test_console_only_when_no_log_dir(self):
        ...
        assert (
            len(file_handlers) == 0
        ), "Should not have any FileHandler when log_dir is None"
    ...
    def test_file_and_console_with_log_dir(self):
        ...
        assert (
            len(file_handlers) > 0
        ), "Should have FileHandler when log_dir is provided"
    ...
    def test_log_file_created(self):
        ...
        assert log_file.name.startswith(
            "extractor_responses_"
        ), "Log file should have correct prefix"
        assert log_file.name.endswith(".log"), "Log file should have .log extension"
        ...
        try:
            ...
            pytest.fail(
                f"Datetime part '{datetime_part}' doesn't match format YYYY-MM-DD_HH-MM-SS"
            )
    ...
    def test_logger_setup_multiple_calls_same_instance(self):
        ...
        assert (
            handler_count_2 == handler_count_1
        ), "Multiple setup calls should not add duplicate handlers"
        assert (
            handler_count_2 <= 2
        ), f"Should have at most 2 handlers, got {handler_count_2}"
