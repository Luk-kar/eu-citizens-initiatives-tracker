...
class TestCreatedFiles:
    ...
    def test_log_file_naming_pattern(self):
        ...
        timestamp_part = log_filename.replace("extractor_responses_", "").replace(
            ".log", ""
        )
        ...
        try:
            ...
            pytest.fail(
                f"Timestamp part '{timestamp_part}' doesn't match format YYYY-MM-DD_HH-MM-SS"
            )
    ...
    def test_log_file_contains_entries(self, program_root_dir):
        ...
        test_data_dir = (
            program_root_dir / "tests" / "data" / "example_htmls" / "responses"
        )
        ...
        assert (
            "Starting ECI responses" in log_content or "Processing" in log_content
    ...
    def test_processor_raises_error_with_zero_responses(self):
        ...
        assert "No HTML response files found" in str(
            exc_info.value
        ...
        assert (
            len(log_files) > 0
        ), "Log file should be created even when processing fails"

        log_content = log_files[0].read_text(encoding="utf-8")
        assert (
            "Starting ECI responses" in log_content
        ), "Log should contain processing start message"
        assert (
            "Processing session:" in log_content
