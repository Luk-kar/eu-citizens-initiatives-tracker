...
class TestDirectoryStructure:
    ...
    def test_response_files_in_year_subdirectories(
        ...
        # Act - Save files for different years (reduced from 4 to 2 for speed)
        years_and_regs = [
            ("2019", "000007"),
            ("2020", "000001"),
        ]
        ...
        for year, reg_number in years_and_regs:
            ...
            assert expected_file.exists(), f"Response file not found: {expected_file}"
    ...
    def test_csv_file_created(self, temp_base_dir):
        ...
        # Verify the CSV file is in the responses directory
        csv_files = list(responses_dir.glob("*.csv"))
        assert len(csv_files) > 0, "No CSV files found in responses directory"
        assert csv_path in csv_files, "Created CSV file is not in responses directory"
    ...
    def test_multiple_year_directories_coexist(
        ...
        # Reduced from 5 years to 3 for speed
