...
# Local imports
from ECI_initiatives.data_pipeline.scraper.responses.statistics import (
    ScrapingStatistics,
    display_completion_summary,
)
from ECI_initiatives.data_pipeline.scraper.responses.consts import LOG_MESSAGES
...
class TestCompletionSummary:
    ...
    def test_zero_failures_reported_on_success(
        self, sample_response_links_all_successful
    ):
        """
        ...
        assert any(
            LOG_MESSAGES["scraping_complete"] in str(call) for call in info_calls
        ), f"Should log '{LOG_MESSAGES['scraping_complete']}' message"

        # Check that "all downloads successful" message is logged
        assert any(
            LOG_MESSAGES["all_downloads_successful"] in str(call) for call in info_calls
        ), f"Should log '{LOG_MESSAGES['all_downloads_successful']}' message"

        # Verify no failure warnings
        assert (
            mock_logger.warning.call_count == 0
        ), "Should not log warnings for successful run"

    def test_summary_shows_correct_counts(
        self, sample_response_links_mixed, sample_failed_urls
    ):
        """
        ...
        assert any(
            expected_total_msg in str(call) for call in info_calls
        ), f"Should log '{expected_total_msg}'"

        # Verify pages downloaded message format
        expected_pages_msg_partial = f"{successful}/{total_links}"
        assert any(
            expected_pages_msg_partial in str(call) for call in info_calls
        ), f"Should log download count '{expected_pages_msg_partial}'"

        # Verify failed downloads warning
        expected_failed_msg = LOG_MESSAGES["failed_downloads"].format(
            failed_count=failed_count
        )
        assert any(
            expected_failed_msg in str(call) for call in warning_calls
    ...
    def test_summary_includes_save_path(self, sample_response_links_all_successful):
        ...
        stats = ScrapingStatistics(
            start_scraping="2024-10-09 14:00:00", responses_dir=save_path
        )
        stats.logger = mock_logger
        ...
        assert any(
            expected_path_msg in str(call) for call in info_calls
    ...
    def test_all_downloads_successful_message_displayed(
        ...
        mock_logger = Mock()
        stats = ScrapingStatistics(
            start_scraping="2024-10-09 14:00:00", responses_dir="/data/test"
        )
        stats.logger = mock_logger
    ...
    def test_statistics_display_header_with_timestamps(self):
        ...
        stats = ScrapingStatistics(
            start_scraping=start_time, responses_dir="/data/test"
        )
        stats.logger = mock_logger

        # Act
        with patch(
            "ECI_initiatives.data_pipeline.scraper.responses.statistics.datetime"
        ) as mock_datetime:
            ...
            stats.display_completion_summary(
                response_links=[], failed_urls=[], downloaded_count=0
            )
        ...
        expected_start_msg = LOG_MESSAGES["start_time"].format(
            start_scraping=start_time
        )
        assert any(
            expected_start_msg in str(call) for call in info_calls
        ), f"Should log start time message: '{expected_start_msg}'"

        # Verify completion time message
        expected_completion_msg = LOG_MESSAGES["completion_timestamp"].format(
            timestamp=completion_time
        )
        assert any(
            expected_completion_msg in str(call) for call in info_calls
        ...
        assert (
            divider_count >= 2
        ), "Should log at least 2 divider lines (header and footer)"
