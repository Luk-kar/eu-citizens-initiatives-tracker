...
class TestLinkExtraction:
    ...
    def test_only_initiatives_with_response_links_included(
        ...
        (year_dir / "2019_000007_en.html").write_text(initiative_html_with_response_link, encoding="utf-8")
        
        # File without response link
        (year_dir / "2019_000008_en.html").write_text(initiative_html_without_response_link, encoding="utf-8")
    ...
    def test_registration_number_and_year_extracted(self, temp_pages_dir, initiative_html_with_response_link):
        ...
        for year, filename in test_files:
            ...
            year_dir.mkdir()
            (year_dir / filename).write_text(initiative_html_with_response_link, encoding="utf-8")
    ...
    def test_all_year_directories_processed(
        ...
        # Assert
        assert len(response_links) == 3, f"Should extract links from all 3 years, but found {len(response_links)}"
        
        extracted_years = {link['year'] for link in response_links}
        assert extracted_years == set(years), \
