...
class TestCreatedFiles:
    ...
    def setup_class(cls, data_dir=None):
        ...
        if not cls.source_timestamp_dir.exists():
            raise FileNotFoundError(
                f"Source timestamp directory not found: {cls.source_timestamp_dir}"
            )
        ...
        if not cls.temp_dir.exists():
            raise FileNotFoundError(f"Temporary directory not found: {cls.temp_dir}")
        ...
        if not cls.test_timestamp_dir.exists():
            raise FileNotFoundError(
                f"Test timestamp directory not found: {cls.test_timestamp_dir}"
            )
        ...
        if not cls.test_pages_dir.exists():
            raise FileNotFoundError(
                f"Test pages directory not found: {cls.test_pages_dir}"
            )
        ...
        def mock_download_limited(self, url, year, reg_number, max_retries=3):
            ...
            success, timestamp = original_download(
                self, url, year, reg_number, max_retries
            )
        ...
        with patch.object(
            ...
            with patch.object(
                ResponseDownloader, "download_single_response", mock_download_limited
            ):
        ...
        missing = [
            ...
            for p in [
                cls.data_dir,
                cls.source_timestamp_dir,
                cls.source_pages_dir,
                cls.temp_dir,
                cls.test_timestamp_dir,
                cls.test_pages_dir,
                cls.responses_dir,
                cls.logs_dir,
            ]
    ...
    def test_responses_directory_structure_created(self):
        ...
        assert (
            self.responses_dir.exists()
        ), f"Responses directory not created: {self.responses_dir}"
        assert self.responses_dir.is_dir(), "Responses path is not a directory"

        # Check CSV file exists
        assert self.csv_file.exists(), f"CSV file not created: {self.csv_file}"
    ...
    def test_response_links_extracted_correctly(self):
        ...
        response_links = responses_main._extract_response_links(
            str(self.test_pages_dir)
        )
        ...
        for link in response_links:
            for field in required_fields:
                assert field in link, f"Response link missing required field: {field}"

        # Verify year directories were scanned
        years_found = {link["year"] for link in response_links}
        assert len(years_found) > 0, "No years found in response links"
    ...
    def test_csv_file_structure_and_content(self):
        ...
        """
        # Check CSV exists
        assert self.csv_file.exists(), "CSV file not created"
    ...
    def test_response_html_files_downloaded(self):
        ...
        assert len(html_files_found) > 0, "No HTML response files downloaded"

        assert (
            len(html_files_found) <= MAX_RESPONSE_DOWNLOADS_E2E_TEST
        ...
        for html_file in html_files_found:
            ...
            assert re.match(
                RESPONSE_PAGE_FILENAME_PATTERN, html_file.name
    ...
    def test_completion_summary_accuracy(self):
        ...
        assert (
            html_file_count == successful_downloads
