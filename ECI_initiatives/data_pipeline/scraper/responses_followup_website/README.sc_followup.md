...
> 
> `initiatives` scraper → `responses` scraper → **this scraper**
> 
...
2. **URL Extraction**:
   ...
   - Normalizes registration numbers (e.g., `YYYY/NNNNNN` → `YYYY_NNNNNN` for filenames).
...
```text
data/
└── YYYY-MM-DD_HH-MM-SS/         # Existing timestamped folder
    ├── logs/
    │   └── scraper_responses_followup_website_....log  # New log file
    ├── responses/               # (Read-only) Input files
    └── responses_followup_website/  # NEW: Downloaded follow-up HTMLs
        ├── 2019/
        │   └── 000007_en.html
        └── ...
```
