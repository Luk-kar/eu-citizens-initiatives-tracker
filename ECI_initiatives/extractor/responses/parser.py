...
class LegislativeOutcomeExtractor(BaseExtractor):
    ...
    def extract_proposal_commitment_deadline(self, soup: BeautifulSoup) -> Optional[str]:
        """
        Extract the deadline by which Commission committed to table a legislative proposal.
        Only captures deadlines for LEGISLATIVE PROPOSALS, not other types of commitments.
        
        Format: YYYY-MM-DD (last day of month/year)
        Examples:
            - "May 2018" -> "2018-05-31"
        ...
        Returns:
            Deadline string in YYYY-MM-DD format or None if no legislative proposal commitment found
        ...
        try:
            ...
            # Check if there's a legislative proposal commitment first
            matcher = self._get_classifier(soup)
            if not matcher.check_committed():
                return None
            
            # Patterns specifically for LEGISLATIVE PROPOSAL deadlines
            legislative_deadline_patterns = [
                ...
                # "communicated its intention to table a legislative proposal, by the end of 2023"
                r'intention to table a legislative proposal[,\s]+by\s+([^.,;]+)',
                
                # "to table a legislative proposal by the end of 2023"
                r'to table a legislative proposal[,\s]+by\s+([^.,;]+)',
                
                # "committed to table a legislative proposal by May 2018"
                r'committed to table a legislative proposal[,\s]+by\s+([^.,;]+)',
                
                # "will table a legislative proposal by the end of 2023"
                r'will table a legislative proposal[,\s]+by\s+([^.,;]+)',
                
                # "to propose legislation by March 2026"
                r'to propose legislation[,\s]+by\s+([^.,;]+)',
                
                # "legislative proposal, by [date], to [action]"
                r'legislative proposal[,\s]+by\s+([^.,;]+?)[,\s]+(?:to|for)',
            ]
            ...
            for sibling in answer_section.find_next_siblings():
                ...
                # Check each pattern for legislative proposal deadlines
                for pattern in legislative_deadline_patterns:
                    match = re.search(pattern, text_lower, re.IGNORECASE)
                    if match:
                        deadline_text = match.group(1).strip()
                        
                        if deadline_text:
                            # Clean and convert the deadline to YYYY-MM-DD
            ...
            raise ValueError(
                f"Error extracting proposal commitment deadline for {self.registration_number}: {str(e)}"
            ) from e
    ...
    def _convert_deadline_to_date(self, deadline: str) -> Optional[str]:
        ...
        Args:
            deadline: Cleaned deadline text like "may 2018", "the end of 2023", "march 2026"
        ...
        end_of_match = re.match(r'(?:the\s+)?end\s+of\s+(\d{4})', deadline_lower)
        if end_of_match:
            year = int(end_of_match.group(1))
            return f"{year}-12-31"
        
        # Pattern 2: "Month YYYY" (e.g., "May 2018", "march 2026")
        month_year_match = re.match(r'(\w+)\s+(\d{4})', deadline_lower)
        if month_year_match:
    ...
    def parse_file(self, html_path: Path, responses_list_data: Dict) -> Optional[ECICommissionResponseRecord]:
        ...
        try:
            ...
            response = ECICommissionResponseRecord(
                ...
                commission_presentation_new_law_deadline=self.legislative_outcome.extract_proposal_commitment_deadline(soup),
