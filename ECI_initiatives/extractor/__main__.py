...
class ECIHTMLParser:
    ...
    def parse_html_file(self, file_path: Path) -> Optional[ECIInitiative]:
        ...
        try:
            ...
            initiative_data = ECIInitiative(
                ...
                organisers=self._extract_organisers_as_json(soup, file_path, title, url),
    ...
    def _extract_organisers_as_json(self, soup: BeautifulSoup, file_path: Path, title: str, url: str) -> Optional[str]:
        """Extract organiser data and return as JSON string"""
        
        try:
            organisers_data = self.extract_organisers_data(soup)

            if organisers_data:
                return json.dumps(organisers_data, ensure_ascii=False, separators=(',', ':'))

        except Exception as e:
            self.logger.error(
                f"Error extracting organisers data - "
